...
plotSpectraRelative <- function(object1, object2, ...) {
  ...
  ggplot(sf,
    ...
    geom_line() +
    labs(x = "Weight [g]", y = "Two Point Relative Difference in Abundance ") +
    scale_x_log10() +
...
#find MSSY for BH a=0.25 on an initially unfished system 
a=0.25
TBXBH_025<- TBst
...
#MSSY for TB fished 
TBKE <- TBst
TBKE@gear_params$knife_edge_size <- TBXKE@species_params$w_mat
TBKE@species_params$knife_edge_size <- TBXKE@species_params$w_mat
gear_params(TBKE)$gear <- "maturity"
species_params(TBKE)$gear <- "maturity"

#Identifying c for MSSY KE
efforts <- seq(from=20.1, to=20.5, by=0.1) 
Fmort <- TBKE
TBKEresults <- MSSY(efforts, Fmort) #c= 20.2, MSSY= 0.02179049

#projection for KE historically fished MSSY:
TBKEMSSY <- TBKE
TBKEMSSY<- setFishing(TBKEMSSY, initial_effort = 20.2)
TBKEMSSYst <- projectToSteady(TBXKEMSSY)
TBKEMSSYpr <- project(TBKEMSSYst, t_max=100)
TotalYield((TBXKEMSSYpr))[101] #0.02179049
#for later plots
TBMSSYTotalBiomassKE <- TotalBiomass((TBKEMSSYpr))
TBMSSYTotalAbundanceKE <-TotalAbundance(TBKEMSSYpr)
TBMSSYTotalYieldKE <- TotalYield((TBKEMSSYpr))
...
TBMSSYdf <- data.frame(policy=rep(c('0', "0.25", "0.5", "0.75", "1", "SaE(mat)"),2), Historically =rep(c("Exploited","Unexploited"),each=6),
...
TBMSSYYield <- ggplot(TBMSSYdf, aes(x=policy, y=Yield, fill=Historically)) + 
  geom_bar(stat="identity", position=position_dodge()) +coord_cartesian(ylim=c(0.018,0.022))+ labs(x=NULL, y='Yield /gm^-3year^-1') +theme(legend.position="none")
...
matplot(cbind(TBXMSSYTotalYieldKE, TBXMSSYTotalYield0, TBXMSSYTotalYield025, TBXMSSYTotalYield05, TBXMSSYTotalYield075, TBXMSSYTotalYield1),
        log = "xy", type = "l", main = "Historically Unfished",
        xlab = "Year", ylab = "Yield /gm^-3year^-1",  col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)
...
matplot(cbind(TBMSSYTotalYieldKE, TBMSSYTotalYield0, TBMSSYTotalYield025, TBMSSYTotalYield05, TBMSSYTotalYield075, TBMSSYTotalYield1),
        log = "xy", type = "l", main = "Historically Fished",
...
legend(x='right',legend = c('SaE(mat)','BH(a=0)','BH(a=0.25)', 'BH(a=0.5)','HB(a=0.75)', 'BH(a=1)'), col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1), lwd=3, cex=1)
...
#############################################################################
#Now we are analyzing fishing efforts which result in the same yield regardless of policy to compare policies rather than system initial conditions in a TB model
...
#yield=0.01 projection for a=0 (productivity)
a=0
TB01BH_0<- TB01KEst
...
TotalYield(TB01BH_0pr)[101] #0.01
#later plots 
...
#yield=0.01 projection for a=0.25 (intermediate)
a<-0.25
TB01BH_025<- TB01KEst
...
TB01RelativeBiomass0df <- data.frame(RelativeBiomass=((TB01Biomass0[101,]-TB01BiomassKE[101,])/TB01BiomassKE[101,]), species=1:12)
TB01RelativeBiomass025df <- data.frame(RelativeBiomass=((TB01Biomass025[101,]-TB01BiomassKE[101,])/TB01BiomassKE[101,]), species=1:12)
TB01RelativeBiomass05df <- data.frame(RelativeBiomass=((TB01Biomass05[101,]-TB01BiomassKE[101,])/TB01BiomassKE[101,]), species=1:12)
TB01RelativeBiomass075df <- data.frame(RelativeBiomass=((TB01Biomass075[101,]-TB01BiomassKE[101,])/TB01BiomassKE[101,]), species=1:12)
TB01RelativeBiomass1df <- data.frame(RelativeBiomass=((TB01Biomass1[101,]-TB01BiomassKE[101,])/TB01BiomassKE[101,]), species=1:12)

TB01Biomass0plot <- ggplot(TB01RelativeBiomass0df, aes(x = factor(species)))+geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
  labs(x = NULL, y = 'Relative Biomass') +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+
  geom_hline(yintercept = ((TB01TotalBiomass0[101]-TB01TotalBiomassKE[101])/TB01TotalBiomassKE[101]),  linetype = "dashed", color = "red")+
...
TB01Biomass025plot <- ggplot(TB01RelativeBiomass025df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2)+ 
  labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
  geom_hline(yintercept = ((TB01TotalBiomass025[101]-TB01TotalBiomassKE[101])/TB01TotalBiomassKE[101]), linetype = "dashed", color = "red") +
  scale_y_continuous(limits = c(-0.25, 1), breaks = seq(-0.25, 1, 0.25))

TB01Biomass05plot <- ggplot(TB01RelativeBiomass05df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
  labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
  geom_hline(yintercept = ((TB01TotalBiomass05[101]-TB01TotalBiomassKE[101])/TB01TotalBiomassKE[101]), linetype = "dashed", color = "red") +
  scale_y_continuous(limits = c(-0.25, 1), breaks = seq(-0.5, 1, 0.25))

TB01Biomass075plot <- ggplot(TB01RelativeBiomass075df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
  labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
  geom_hline(yintercept = ((TB01TotalBiomass075[101]-TB01TotalBiomassKE[101])/TB01TotalBiomassKE[101]), linetype = "dashed", color = "red")+ 
  scale_y_continuous(limits = c(-0.25, 1), breaks = seq(-0.25, 1, 0.25))

TB01Biomass1plot <- ggplot(TB01RelativeBiomass1df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
  labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
  geom_hline(yintercept = ((TB01TotalBiomass1[101]-TB01TotalBiomassKE[101])/TB01TotalBiomassKE[101]), linetype = "dashed", color = "red") +
  scale_y_continuous(limits = c(-0.25, 1), breaks = seq(-0.25, 1, 0.25))

TB01RelativeAbundance0df <- data.frame(RelativeAbundance=((TB01Abundance0[101,]-TB01AbundanceKE[101,])/TB01AbundanceKE[101,]), species=1:12)
TB01RelativeAbundance025df <- data.frame(RelativeAbundance=((TB01Abundance025[101,]-TB01AbundanceKE[101,])/TB01AbundanceKE[101,]), species=1:12)
TB01RelativeAbundance05df <- data.frame(RelativeAbundance=((TB01Abundance05[101,]-TB01AbundanceKE[101,])/TB01AbundanceKE[101,]), species=1:12)
TB01RelativeAbundance075df <- data.frame(RelativeAbundance=((TB01Abundance075[101,]-TB01AbundanceKE[101,])/TB01AbundanceKE[101,]), species=1:12)
TB01RelativeAbundance1df <- data.frame(RelativeAbundance=((TB01Abundance1[101,]-TB01AbundanceKE[101,])/TB01AbundanceKE[101,]), species=1:12)

TB01Abundance0plot <- ggplot(TB01RelativeAbundance0df, aes(x = factor(species)))+ geom_point(aes(y = RelativeAbundance), shape=19, size=2) + 
  labs(x = 'Species', y = 'Relative Abundance') +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+
  geom_hline(yintercept = ((TB01TotalAbundance0[101]-TB01TotalAbundanceKE[101])/TB01TotalAbundanceKE[101]) , linetype = "dashed", color = "blue") +
  scale_y_continuous(limits = c(-0.2, 1.1), breaks = seq(-0.2, 1.1, 0.2))

TB01Abundance025plot <- ggplot(TB01RelativeAbundance025df, aes(x = factor(species)))+ geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
  labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") + 
  geom_hline(yintercept = ((TB01TotalAbundance025[101]-TB01TotalAbundanceKE)/TB01TotalAbundanceKE[101]), linetype = "dashed", color = "blue") +
  scale_y_continuous(limits = c(-0.2, 1.1), breaks = seq(-0.2, 1.1, 0.2))

TB01Abundance05plot <- ggplot(TB01RelativeAbundance05df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
  labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
  geom_hline(yintercept = ((TB01TotalAbundance05[101] - TB01TotalAbundanceKE)/TB01TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+
  scale_y_continuous(limits = c(-0.2, 1.1), breaks = seq(-0.2, 1.1, 0.2))

TB01Abundance075plot<- ggplot(TB01RelativeAbundance075df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
  labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") + 
  geom_hline(yintercept = ((TB01TotalAbundance075[101]-TB01TotalAbundanceKE)/TB01TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+
  scale_y_continuous(limits = c(-0.2, 1.1), breaks = seq(-0.2, 1.1, 0.2))

TB01Abundance1plot<- ggplot(TB01RelativeAbundance1df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
  labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") + 
  geom_hline(yintercept = ((TB01TotalAbundance1[101]-TB01TotalAbundanceKE)/TB01TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+
  scale_y_continuous(limits = c(-0.2, 1.1), breaks = seq(-0.2, 1.1, 0.2))
...
plotSpectraRelative(TB01KEpr,TB01BH_0pr) +ylim(c(-0.5,1))
plotSpectraRelative(TB01KEpr,TB01BH_1pr)+ylim(c(-0.5,1))
plotSpectraRelative(TB01BH_0pr,TB01BH_1pr)

matplot(cbind(TB01TotalYieldKE, TB01TotalYield0, TB01TotalYield025,TB01TotalYield05, TB01TotalYield075, TB01TotalYield1),
        log = "xy", type = "l", main = "TB Model",
        xlab = "Year", ylab = "Yield /gm^-3year^-1",  col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)
...
#finding MSSY for a=0 (productivty)
a=0
NSXBH_0<- NSst
...
#finding MSSY for a=0.5 (intermediate)
a=0.5
NSXBH_05<- NSst
...
#Total yield previously fished under SaE(mat)
NSKE <- NSst
gear_params(NSKE)$gear <- "Knife Edge"
species_params(NSKE)$gear <- "Knife Edge"
gear_params(NSKE)$knife_edge_size <- NSKE@species_params$w_mat
species_params(NSKE)$knife_edge_size <- NSKE@species_params$w_mat

efforts <- seq(from=1.26, to=1.27 , by=0.01) #won't converge >1.27
Fmort <- NSXKE
NSKE_results <- NSMSSY(efforts, Fmort) #c=1.27, Yield=2.875588e+12  

NSMSSYKE <- NSXKE
NSMSSYKEst <- projectToSteady(NSMSSYKE, effort=1.27)
NSMSSYKEpr <- project(NSMSSYKEst, effort=1.27)
TotalYield(NSMSSYKEpr)[101] #2.875588e+12 
NSMSSYTotalYieldKE <- TotalYield(NSMSSYKEpr) 
NSMSSYTotalBiomassKE <-TotalBiomass(NSMSSYKEpr)
NSMSSYTotalAbundanceKE <- TotalAbundance(NSMSSYKEpr)
...
#NS MSSY PLOTS
NSMSSYdf <- data.frame(policy=rep(c("0", "0.25", "0.5", "0.75", "1", "SaE(mat)"),2), Historically =rep(c("Exploited","Unexploited"),each=6),
...
matplot(cbind(NSXMSSYTotalYieldKE, NSXMSSYTotalYield0, NSXMSSYTotalYield025, NSXMSSYTotalYield05, NSXMSSYTotalYield075, NSXMSSYTotalYield1),
        log = "xy", type = "l", main = "Initially Unifished",
        xlab = "Year", ylab = "Yield /gyear^-1",  col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)

matplot(cbind(NSMSSYTotalYieldKE, NSMSSYTotalYield0, NSMSSYTotalYield025, NSMSSYTotalYield05, NSMSSYTotalYield075, NSMSSYTotalYield1),
        log = "xy", type = "l", main = "Initially Fished",
        xlab = "Year", ylab = "Yield /gyear^-1",  col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)
legend(x='topright',legend = c('SaE(mat)','BH(a=0)','BH(a=0.25)', 'BH(a=0.5)','BH(a=0.75)', 'BH(a=1'), col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1), lwd=3, cex=1)
...
matplot(cbind(NSTotalBiomass, NSXMSSYTotalBiomassKE, NSXMSSYTotalBiomass0, NSXMSSYTotalBiomass025, NSXMSSYTotalBiomass05, NSXMSSYTotalBiomass075, NSXMSSYTotalBiomass1),
        log = "xy", type = "l", main = "",
        xlab = "Year", ylab = "Biomass /gmyear^-1",  col = c(8,1,7,3,4,5,2), lty=c(4,2,1,1,1,1,2),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)

matplot(cbind(NSMSSYTotalBiomassKE, NSMSSYTotalBiomass0, NSMSSYTotalBiomass025, NSMSSYTotalBiomass05, NSMSSYTotalBiomass075, NSMSSYTotalBiomass1),
        log = "xy", type = "l", main = "",
        xlab = "Year", ylab = "Biomass /gyear^-1",  col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)
...
matplot(cbind(NSTotalAbundance, NSXMSSYTotalAbundanceKE, NSXMSSYTotalAbundance0, NSXMSSYTotalAbundance025, NSXMSSYTotalAbundance05, NSXMSSYTotalAbundance075, NSXMSSYTotalAbundance1),
        log = "xy", type = "l", main = "",
        xlab = "Year", ylab = "Abundance /year^-1", col = c(8,1,7,3,4,5,2), lty=c(4,2,1,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)

matplot(cbind(NSMSSYTotalAbundanceKE, NSMSSYTotalAbundance0, NSMSSYTotalAbundance025, NSMSSYTotalAbundance05, NSMSSYTotalAbundance075, NSMSSYTotalAbundance1),
        log = "xy", type = "l", main = "",
        xlab = "Year", ylab = "Abundance /year^-1", col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2)
...
matplot(cbind(NSe12TotalYieldKE, NSe12TotalYield0, NSe12TotalYield025, NSe12TotalYield05, NSe12TotalYield075, NSe12TotalYield1),
        log = "xy", type = "l", main = "NS Model",
        xlab = "Year", ylab = "Yield /gyear-1", col = c(1,7,3,4,5,2), lty=c(4,1,2,2,2,1),lwd = 3, bty = "l",cex.axis=1.2,cex.lab=1.2, xlim=c(1,100))
legend(x='topright',legend = c('SaE(mat)', 'BH(a=0)','BH(a=0.25)', 'BH(a=0.5)','BH(a=0.75)', 'BH(a=1)'), lty = c(4,1,2,2,2,1), col = c(1,7,3,4,5,2), lwd=3, cex=1)

#Yield composition plot with species ordered in increasing asympotoic size 
NSe12YieldKEdf <- data.frame(yield100=NSe12YieldKE[101,], yield0=NSe12YieldKE[1,], species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12Yield0df <- data.frame(yield100=NSe12Yield0[101,], yield0=NSe12Yield0[1,], species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12Yield025df <- data.frame(yield100=NSe12Yield025[101,], yield0=NSe12Yield025[1,], species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12Yield05df <- data.frame(yield100=NSe12Yield05[101,], yield0=NSe12Yield05[1,], species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12Yield075df <- data.frame(yield100=NSe12Yield075[101,], yield0=NSe12Yield075[1,], species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12Yield1df <- data.frame(yield100=NSe12Yield1[101,], yield0=NSe12Yield1[1,], species=c(1,2,3,5,4,8,7,6,9,10,12,11))
...
#relative biomass and abundance composition plots with species odered in increasing asympototic size
NSe12RelativeBiomass0df <- data.frame(RelativeBiomass=((NSe12Biomass0[101,]- NSe12BiomassKE[101,])/NSe12BiomassKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeBiomass025df <- data.frame(RelativeBiomass=((NSe12Biomass025[101,]- NSe12BiomassKE[101,])/NSe12BiomassKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeBiomass05df <- data.frame(RelativeBiomass=((NSe12Biomass05[101,]- NSe12BiomassKE[101,])/NSe12BiomassKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeBiomass075df <- data.frame(RelativeBiomass=((NSe12Biomass075[101,]- NSe12BiomassKE[101,])/NSe12BiomassKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeBiomass1df <- data.frame(RelativeBiomass=((NSe12Biomass1[101,]- NSe12BiomassKE[101,])/NSe12BiomassKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
                             
NSe12Biomass0plot <- ggplot(NSe12RelativeBiomass0df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
     labs(x = NULL, y = 'Relative Biomass') +geom_hline(yintercept = 0, linetype = "dotted", color = "black") + 
    geom_hline(yintercept = ((NSe12TotalBiomass0[101] - NSe12TotalBiomassKE)/NSe12TotalBiomassKE[101]), linetype = "dashed", color = "red")+
...
NSe12Biomass025plot <- ggplot(NSe12RelativeBiomass025df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2)+
   labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") +
    geom_hline(yintercept = ((NSe12TotalBiomass025[101]- NSe12TotalBiomassKE)/NSe12TotalBiomassKE[101]), linetype = "dashed", color = "red")+
    scale_y_continuous(limits = c(-0.5, 0.5), breaks = seq(-0.5, 0.5, 0.25))
                             
NSe12Biomass05plot <- ggplot(NSe12RelativeBiomass05df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
    labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+ 
   geom_hline(yintercept = ((NSe12TotalBiomass05[101]-NSe12TotalBiomassKE)/NSe12TotalBiomassKE[101]), linetype = "dashed", color = "red") +
  scale_y_continuous(limits = c(-0.5, 0.5), breaks = seq(-0.5, 0.5, 0.25))
                             
NSe12Biomass075plot <- ggplot(NSe12RelativeBiomass075df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
   labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black") + 
    geom_hline(yintercept = ((NSe12TotalBiomass075[101]-NSe12TotalBiomassKE)/NSe12TotalBiomassKE[101]), linetype = "dashed", color = "red") +
    scale_y_continuous(limits = c(-0.5, 0.5), breaks = seq(-0.5, 0.5, 0.25))
                             
NSe12Biomass1plot <- ggplot(NSe12RelativeBiomass1df, aes(x = factor(species))) + geom_point(aes(y = RelativeBiomass), shape = 19, size = 2) +
   labs(x = NULL, y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+
   geom_hline(yintercept = ((NSe12TotalBiomass1[101]-NSe12TotalBiomassKE)/NSe12TotalBiomassKE[101]), linetype = "dashed", color = "red") +
    scale_y_continuous(limits = c(-0.5, 0.5), breaks = seq(-0.5, 0.5, 0.25))
                             
NSe12RelativeAbundance0df <- data.frame(RelativeAbundance=((NSe12Abundance0[101,]-NSe12AbundanceKE[101,])/NSe12AbundanceKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeAbundance025df <- data.frame(RelativeAbundance=((NSe12Abundance025[101,]-NSe12AbundanceKE[101,])/NSe12AbundanceKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeAbundance05df <- data.frame(RelativeAbundance=((NSe12Abundance05[101,]-NSe12AbundanceKE[101,])/NSe12AbundanceKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeAbundance075df <- data.frame(RelativeAbundance=((NSe12Abundance075[101,]-NSe12AbundanceKE[101,])/NSe12AbundanceKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
NSe12RelativeAbundance1df <- data.frame(RelativeAbundance=((NSe12Abundance1[101,]-NSe12AbundanceKE[101,])/NSe12AbundanceKE[101,]), species=c(1,2,3,5,4,8,7,6,9,10,12,11))
                             
NSe12Abundance0plot <- ggplot(NSe12RelativeAbundance0df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
        labs(x = 'Species', y = 'Relative Abundance') +geom_hline(yintercept = 0, linetype = "dotted", color = "black") + 
        geom_hline(yintercept = ((NSe12TotalAbundance0[101]-NSe12TotalAbundanceKE[101])/NSe12TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+
        scale_y_continuous(limits = c(-0.2, 0.2), breaks = seq(-0.2, 0.2, 0.1))
        
NSe12Abundance025plot <- ggplot(NSe12RelativeAbundance025df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
        labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+
       geom_hline(yintercept = ((NSe12TotalAbundance025[101]-NSe12TotalAbundanceKE[101])/NSe12TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+ 
       scale_y_continuous(limits = c(-0.2, 0.2), breaks = seq(-0.2, 0.2, 0.1))

NSe12Abundance05plot <- ggplot(NSe12RelativeAbundance05df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
       labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+
      geom_hline(yintercept = ((NSe12TotalAbundance05[101]-NSe12TotalAbundanceKE[101])/NSe12TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+
      scale_y_continuous(limits = c(-0.2, 0.2), breaks = seq(-0.2, 0.2, 0.1))

NSe12Abundance075plot<- ggplot(NSe12RelativeAbundance075df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
        labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+
        geom_hline(yintercept = ((NSe12TotalAbundance075[101]-NSe12TotalAbundanceKE[101])/NSe12TotalAbundanceKE[101]), linetype = "dashed", color = "blue")+
        scale_y_continuous(limits = c(-0.2, 0.2), breaks = seq(-0.2, 0.2, 0.1))
                             
NSe12Abundance1plot<- ggplot(NSe12RelativeAbundance1df, aes(x = factor(species))) + geom_point(aes(y = RelativeAbundance), shape = 19, size = 2) +
    labs(x = 'Species', y = NULL) +geom_hline(yintercept = 0, linetype = "dotted", color = "black")+ 
    geom_hline(yintercept = ((NSe12TotalAbundance1[101]-NSe12TotalAbundanceKE[101])/NSe12TotalAbundanceKE[101]), linetype = "dashed", color = "blue") +
    scale_y_continuous(limits = c(-0.2, 0.2), breaks = seq(-0.2, 0.2, 0.1))

plot_grid(NSe12YieldKEplot, NSe12Yield0plot, NSe12Yield025plot, NSe12Yield05plot, NSe12Yield075plot, NSe12Yield1plot, ncol= 3)
plot_grid(NSe12Biomass0plot, NSe12Biomass025plot, NSe12Biomass05plot, NSe12Biomass075plot, NSe12Biomass1plot, NSe12Abundance0plot, NSe12Abundance025plot, NSe12Abundance05plot, NSe12Abundance075plot, NSe12Abundance1plot, ncol = 5)
